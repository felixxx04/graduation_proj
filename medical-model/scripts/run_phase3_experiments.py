"""Phase 3 对比实验脚本 — embed_dim=8 + 16-field schema + DeepSeek验证数据

核心改动 (vs Phase 2):
1. embed_dim: 16→8 (匹配当前config, 减少参数提升样本/参数比率)
2. 数据: 使用DeepSeek指征验证后的1815药物完整数据
3. 活跃模型: 训练结果将部署为saved_models/best_model.pt

3组核心实验:
  1. no_dp_baseline_v6   — embed_dim=8, 无DP训练 (v6 baseline)
  2. dp_finetune_eps1_v6  — 预训练→DP微调 ε=1.0 (主要实验组)
  3. dp_finetune_eps05_v6 — 预训练→DP微调 ε=0.5 (严格隐私)

输出:
  experiments/results/phase3/ 下每组 metadata + train_history
  experiments/results/phase3/comparison_table.json 汇总表
"""
...
RESULTS_DIR = PROJECT_ROOT / "experiments" / "results" / "phase3"
...
def run_no_dp_baseline(name, pipeline_result, epochs=20) -> Dict[str, Any]:
    """运行无DP基线实验 (embed_dim=8, 16-field schema)"""
    ...
    logger.info(f"Starting experiment: {name} (No-DP, embed_dim={settings.embed_dim}, field_dims={field_dims})")
    ...
    metadata['schema_version'] = 'v6_embeddim8_16fields'
...
def build_comparison_table(all_results: List[Dict[str, Any]]) -> None:
    ...
    # 打印对比表
    print("\n" + "=" * 100)
    print("Phase 3 对比实验结果 (embed_dim=8 + 16-field schema + DeepSeek verified data)")
    print("=" * 100)
    header = (
...
def main():
    ...
    logger.info("Running pipeline (embed_dim=8, 16-field schema, 1815 drugs)...")
    ...
    # ── 实验1: No-DP Baseline (v6, embed_dim=8) ──
    result1 = run_no_dp_baseline(
        name="no_dp_baseline_v6",
        pipeline_result=pipeline_result,
        epochs=20,
    )
    all_results.append(result1)

    pretrained_model_path = str(RESULTS_DIR / "no_dp_baseline_v6" / "best_model.pt")

    # ── 实验2: DP finetune ε=1.0 (v6) ──
    result2 = run_dp_finetune(
        name="dp_finetune_eps1_v6",
        pipeline_result=pipeline_result,
    ...
    # ── 实验3: DP finetune ε=0.5 (v6) ──
    result3 = run_dp_finetune(
        name="dp_finetune_eps05_v6",
        pipeline_result=pipeline_result,
        pretrained_model_path=pretrained_model_path,
    ...
    logger.info("All Phase 3 experiments completed!")
